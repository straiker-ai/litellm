...
def test_init_rejects_bad_sampling_rate():
    with pytest.raises(ValueError):
        StraikerGuardrail(api_key="k", streaming_sampling_rate=0)
...
def test_streaming_flags_default_to_buffered():
    g = _make_guardrail()
...
@pytest.mark.asyncio
async def test_no_smart_publish_header_or_field():
    g = _make_guardrail()
    g.async_handler.post.return_value = _mock_response("NONE")
    await g.apply_guardrail(
    ...
    headers = g.async_handler.post.call_args.kwargs["headers"]
    assert not any("smart" in k.lower() for k in headers)
    assert "smart_publish" not in _posted_payload(g)
