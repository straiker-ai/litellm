...
class StraikerGuardrail(CustomGuardrail):
    ...
    def __init__(
        ...
        **kwargs: object,
        ...
        self.streaming_end_of_stream_only = (
            True if streaming_end_of_stream_only is None else streaming_end_of_stream_only
        )
        self.streaming_sampling_rate = 5 if streaming_sampling_rate is None else streaming_sampling_rate
        self.streaming_buffer_until_moderated = (
            True if streaming_buffer_until_moderated is None else streaming_buffer_until_moderated
        )
    ...
    async def _post_webhook(self, payload: dict) -> tuple[StraikerWebhookResponse | None, str | None]:
        try:
            body_bytes = len(json.dumps(payload, default=str).encode("utf-8"))
        except (TypeError, ValueError, OverflowError) as error:
            return None, f"request serialization failed: {error}"
        if body_bytes > self.max_payload_bytes:
            return None, f"payload {body_bytes}B exceeds max_payload_bytes {self.max_payload_bytes}"
            ...
            try:
                ...
                if resp.status_code == 200:
                    try:
                        ...
                        return None, f"invalid response schema: {ve}"
                    if self.verbose:
                        verbose_proxy_logger.info(
                            json.dumps(
                                {
    ...
    async def apply_guardrail(
        ...
        parsed, err = await self._post_webhook(payload)
        if err is not None:
            is_unreachable = err.startswith(tuple(f"HTTP {code}" for code in UNREACHABLE_STATUS)) or not err.startswith(
                "HTTP "
            )
            return self._fail(inputs=inputs, input_type=input_type, error=err, is_unreachable=is_unreachable)
