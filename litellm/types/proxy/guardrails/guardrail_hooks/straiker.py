...
class StraikerGuardrailConfigModelOptionalParams(BaseModel):
    ...
    streaming_end_of_stream_only: bool | None = Field(
        default=None,
        description=(
            "Run the guardrail once at end of stream over the assembled response. Defaults to True "
            "so streamed output is evaluated as a single assembled response."
        ),
    )
    streaming_sampling_rate: int | None = Field(
        default=None,
        ge=1,
        description="When not buffering, run the guardrail every Nth streamed chunk. Must be >= 1 when set.",
    )
    streaming_buffer_until_moderated: bool | None = Field(
        default=None,
        description=(
            "Withhold every streamed chunk until end-of-stream moderation passes, so no flagged chunk "
            "reaches the client before a block. Defaults to True."
        ),
    )
